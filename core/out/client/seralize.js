...
import { isAdmin as is_admin, parse_content, getAlternateId, get_content_type, extract_text_from_message, } from "../util";
const serialize = async (msg, client) => {
    ...
    return {
        ...
        media: Boolean(message?.imageMessage ||
            message?.videoMessage ||
            message?.audioMessage ||
            message?.documentMessage ||
            message?.stickerMessage),
        quoted: quoted && quotedMessage && quotedType
            ? {
                key: {
        ...
        reply: async function (text) {
            return await client.sendMessage(this.chat, { text }, { quoted: this?.quoted || msg });
        },
        ...
        forward: async function (jid, msg, opts) {
