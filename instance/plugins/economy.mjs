...
export default [
    {
        ...
        function: async (msg, args) => {
            const userId = args ? (await parseId(msg, args) || msg.sender) : msg.sender;
    ...
    },
    {
        ...
        function: async (msg, args) => {
            ...
            if (amount > eco.balance) return msg.reply("You don't have enough money in your wallet.");
    ...
    },
    {
        ...
        function: async (msg, args) => {
            ...
            if (amount > eco.bank) return msg.reply("You don't have enough money in your bank.");
    ...
    },
    {
        ...
        function: async (msg, args) => {
            ...
            if (amount > ecoSender.balance) return msg.reply("You don't have enough money.");
    ...
    },
    {
        ...
        function: async (msg) => {
            ...
            if (diff < cooldown) {
                ...
                return msg.reply(`🕒 You can claim your daily reward in ${hours}h ${minutes}m.`);
            }
            ...
            return msg.reply(`💰 Check-in successful! You received ${formatMoney(reward)}.`);
        }
    },
    {
        pattern: "work",
        ...
        function: async (msg) => {
            ...
            const assignments = [
                "You fixed a bug.", "You walked a dog.", "You sold lemonade.", "You helped a grandma.", "You cleaned the house."
            ];
    ...
    },
    {
        ...
        function: async (msg, args) => {
            ...
            const eco = await EconomyManager.get(msg.session, msg.sender);
            if (amount > eco.balance) return msg.reply("You don't have enough money.");
            ...
            if (win) {
                ...
                return msg.reply(`🎲 You won ${formatMoney(amount)}! Balance: ${formatMoney(BigInt(eco.balance) + BigInt(amount))}`);
            } else {
                await EconomyManager.update(msg.session, msg.sender, {
                    balance: BigInt(eco.balance) - BigInt(amount)
                });
                return msg.reply(`🎲 You lost ${formatMoney(amount)}. Balance: ${formatMoney(BigInt(eco.balance) - BigInt(amount))}`);
            }
        }
    },
    {
        ...
        function: async (msg, args) => {
            ...
            if (target === msg.sender) return msg.reply("You can't rob yourself (that's just spending).");

            // Cooldown 1h
            const now = new Date();
            const last = eco.lastRob ? new Date(eco.lastRob) : new Date(0);
            if ((now - last) < 3600000) {
                return msg.reply("You are laying low from the police. Try again later.");
            }
            ...
            if (success) {
                ...
                return msg.reply(`🚓 You got caught! You paid a fine of ${formatMoney(fine)}.`);
            }
        }
    }
]
