            ...
            if (game) {
                if (game.state === 'WAITING') {
                    if (game.players.some(p => p.id === sender)) {
                        return msg.reply("You are already in the game. Waiting for opponent...");
                    }
                ...
                msg.reply("Tic-Tac-Toe started! Waiting for player 2 (30s). Reply `!ttt` to join.");
                ...
                game.timer = setTimeout(() => {
                    ...
                    if (g && g.state === 'WAITING') {
                        g.addCPU();
                        g.state = 'PLAYING';
                        msg.client.sendMessage(chatId, { text: "No player joined. Playing against CPU.\n" + g.renderBoard() });
                    }
                }, 30000);
                return;
            }
...
async function handleMove(msg, game, index) {
    ...
    if (result) {
        ...
        if (result === 'draw') return msg.reply(`Game Draw!\n${game.renderBoard()}`);
    ...
    if (nextP.isCPU) {
        ...
        if (move !== -1) {
            ...
            if (cpuResult) {
                games.delete(game.chatId);
                if (cpuResult === 'draw') return msg.client.sendMessage(game.chatId, { text: `Game Draw!\n${game.renderBoard()}` });
                return msg.client.sendMessage(game.chatId, { text: `CPU Wins!\n${game.renderBoard()}` });
            }
            game.nextTurn();
        }
    }

    await msg.client.sendMessage(game.chatId, { text: `Turn: ${parseUser(game.getCurrentPlayer().id)}\n${game.renderBoard()}`, mentions: [game.getCurrentPlayer().id] });
}